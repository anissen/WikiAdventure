...
CEngine::CEngine()
{
	DebugInfo( TypeCreate, "Creating base engine" );
	m_lLastTick			= 0;
	m_lFrameTickValue	= 0;
	m_iWidth			= 800;
	m_iHeight			= 600;
...
CEngine::~CEngine()
{
	DebugInfo( TypeDelete, "Unloading base engine" );
	SDL_Quit();
}
...
{
	char czNewSize[64];
	sprintf( czNewSize, "Setting new size: %d x %d", iWidth, iHeight );
	DebugInfo( TypeInfo, czNewSize );
...
void CEngine::Start()
{
	DebugInfo( TypeInfo, "Main loop started" );
...
{
	DebugInfo( TypeInfo, string( "Setting new title: \"" + string( czTitle ) + "\"" ).c_str() );
	m_czTitle = czTitle;
